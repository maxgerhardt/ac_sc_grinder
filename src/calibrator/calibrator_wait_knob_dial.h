...
#define IS_KNOB_FALSE(val) (val < KNOB_TRESHOLD)
#define IS_KNOB_TRUE(val) (val >= KNOB_TRESHOLD)
...
public:
  ...
  bool tick() {
    ...
    case KNOB_UP_CHECK:
      ...
      if (IS_KNOB_FALSE(knob) && ticks_cnt == 0) break;

      // Finally, knob is up => count time & reset state if too long
      if (IS_KNOB_TRUE(knob))
      {
        ticks_cnt++;
